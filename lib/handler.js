...
╰━━━━━━━━━━━━━━━━━╯`,
    ...
    if (isGroup) {
      try {
        ...
        const meAlt = (sock.user.id.split(":")[0] + "@s.whatsapp.net").replace(/^.*?(\d+)/, "$1");
        const part = meta.participants.find(p => p.id === sender);
        const botPart = meta.participants.find(p => p.id === meId || p.id.startsWith(sock.user.id.split(":")[0]));
        isAdmin = !!(part && (part.admin === "admin" || part.admin === "superadmin"));
        isBotAdmin = !!(botPart && (botPart.admin === "admin" || botPart.admin === "superadmin"));
      } catch {}
    }

    // 🛡️ ANTIBAN DONO: se target for dono, recusa qualquer ação destrutiva
    ...
    if (custom) { await react(sock, msg, "💬"); return reply(custom); }

    // helper pra erros admin reais
    const tryAdmin = async (fn, okMsg) => {
      try {
        ...
        if (m.includes("forbidden") || m.includes("not-authorized") || m.includes("403"))
          return reply(`❌ O bot precisa ser ${bold("admin")} para esta acção.`);
        if (m.includes("not in group")) return reply("❌ Bot não está no grupo.");
        return reply(`❌ Falhou: ${e.message}`);
      }
    };
...
Vez de ❌ (TU). Joga: ${config.prefix}jdvjogar 1-9`
      ...
      // ╔═══════════ AÇÕES ═══════════╗
      case "abraco": case "beijo": case "tapa": case "soco": case "mata":
      ...
      {
        const emo = { abraco:"🤗",beijo:"💋",tapa:"👋",soco:"👊",mata:"💀",reza:"🙏",casar:"💍",
                      ship:"💕",amizade:"🤝",odio:"💢",iq:"🧠",gay:"🏳️‍🌈",burro:"🤡",
                      lindo:"😍",feio:"🥴",gostosa:"🔥",fofoqueiro:"🗣️" }[cmd];
        ...
        return reply(`${emo} @${senderNum} ${cmd} @${target.split("@")[0]}`,{mentions:[sender,target]});
      }
      case "chorar": case "rir": case "dancar": case "dormir":
        return reply({chorar:"😭",rir:"🤣",dancar:"💃",dormir:"😴"}[cmd]+` @${senderNum}`,{mentions:[sender]});
      ...
      default:
        await react(sock, msg, "❓");
        return reply(`❓ Comando *${rawCmd}* não existe.\n💡 Usa *${config.prefix}menu* para ver tudo.`);
    }
  } catch (e) {
    console.error("handler err:", e);
    try { await sock.sendMessage(msg.key.remoteJid, { text: "⚠️ Erro: "+e.message }, { quoted: msg }); } catch {}
  }
}
